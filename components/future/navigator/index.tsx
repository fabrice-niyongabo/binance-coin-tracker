...
function Navigator() {
  ...
  const handleNext = () => {
    if (selectedFutureMarketData) {
      ...
      setSelectedFutureMarketData(nextCoin);
      setTimeout(() => {
        window.open(
          `https://www.binance.com/en/futures/${nextCoin.symbol}`,
          "_blank"
        );
        setNextLoading(false);
      }, 200);
  ...
  const handlePrevious = () => {
    if (selectedFutureMarketData) {
      ...
      setSelectedFutureMarketData(previousCoin);
      setTimeout(() => {
        window.open(
          `https://www.binance.com/en/futures/${previousCoin.symbol}`,
          "_blank"
        );
        setPreviousLoading(false);
      }, 200);
    ...
    <div className="h-full flex items-center justify-center">
      {!selectedFutureMarketData ? (
        ...
        <div className="flex gap-10 items-center justify-center">
          <button
            disabled={
              selectedFutureMarketData.symbol === futureMarketData[0].symbol ||
              previousLoading
            }
          ...
          <div className="bg-gray-900 rounded-full p-10 flex items-center justify-center min-w-80 gap-2 shadow-md">
            <div>
              {binanceCryptoIcons.has(
                splitSymbol(selectedFutureMarketData.symbol).base.toLowerCase()
              ) ? (
                <Image
                  src={
                    "https://raw.githubusercontent.com/VadimMalykhin/binance-icons/main/crypto/" +
                    splitSymbol(
                      selectedFutureMarketData.symbol
                    ).base.toLowerCase() +
                    ".svg"
                  }
                  alt={splitSymbol(selectedFutureMarketData.symbol).base}
                  width={80}
                  height={80}
                />
                ...
                >
                  <span className="text-white" style={{ fontSize: 10 }}>
                    {selectedFutureMarketData.baseAsset.charAt(0)}
                    {selectedFutureMarketData.quoteAsset.charAt(0)}
                  </span>
                </div>
              )}
            </div>
            <div>
              <div className="text-2xl text-white">
                {selectedFutureMarketData.baseAsset}/
                {selectedFutureMarketData.quoteAsset}
              </div>
              <div
                className={[
                  "text-2xl text-yellow-500",
                  futureMarketPrices[selectedFutureMarketData.symbol]
                    ?.direction === "up"
                    ? "text-green-500"
                    : "",
                  futureMarketPrices[selectedFutureMarketData.symbol]
              ...
              >
                {futureMarketPrices[selectedFutureMarketData.symbol]?.price ||
                  0}
              </div>
            </div>
          </div>
          <button
            disabled={
              selectedFutureMarketData.symbol ===
                futureMarketData[futureMarketData.length - 1].symbol ||
              nextLoading
            }
            className="bg-gray-900 rounded-full p-10 flex items-center justify-center hover:bg-yellow-500 transition-all duration-800 hover:cursor-pointer shadow-md disabled:opacity-30 disabled:cursor-not-allowed"
            onClick={handleNext}
